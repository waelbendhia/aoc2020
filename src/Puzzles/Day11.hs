...
stepSeatPlan sp
  ...
  where
    ...
    newState i j
        = let adj = length $ filter (== Occupied) $ catMaybes $ (sp !!?)
                  <$> getSurrounding i j in case sp !!! ( i, j ) of
            Floor    -> Floor
...
occupiedSeatsAtEnd i = length $ filter (== Occupied) $ V.toList =<< V.toList
    (stepSeatPlan i)
...
stepSeatPlan' sp
  ...
  where
    ...
    newState :: Int -> Int -> SeatState
    newState i j = let vis = getVisible i j in case sp !!! ( i, j ) of
        Floor    -> Floor
