...
testInput = concat $ either (error . show) id $
    mapM (parse parseInstruction "") ["F10", "N3", "F7", "R90", "F11"]

part1 = abs x + abs y
  where
    (_, x, y) = executeInstructions is
    is = concat $ either (error . show) id $
        mapM (parse parseInstruction "") input

executeInstructions' is = go is (10, 1) 0 0
  where
    go (Move c a : is) (wx, wy) x y =
        go is
           (case c of N -> (wx, wy + a)
                      S -> (wx, wy - a)
                      E -> (wx + a, wy)
    ...
    go (Forward i : is) (wx, wy) x y = go is (wx, wy) (x + i * wx) (y + i * wy)
    go [] _ x y = (x, y)
