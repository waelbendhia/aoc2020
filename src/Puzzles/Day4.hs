...
passportFieldMapParser = M.fromList
    <$> many (passportWithFieldValueParser <* (() <$ space <|> eof))

data Passport = Passport { byr :: Text
                         , iyr :: Text
                         , eyr :: Text
                         , hgt :: Text
                         , hcl :: Text
...
part1 :: Int
part1 = length $ filter isJust $ either (error . show) id $
    mapM (parse passportParser "") sanitizedInput
...
boundedInt mi ma = do
    ...
    when (d < mi || d > ma)
         (unexpected $
          Prelude.unwords [d', "must be between", show mi, "and", show ma])
    pure $ pack d'
...
passportFieldMapParserWithValidation
    :: Parsec String () (M.Map PassportField Text)
passportFieldMapParserWithValidation = M.fromList
    <$> many (passportWithFieldValueParserWithValidation
              <* (() <$ space <|> eof))
