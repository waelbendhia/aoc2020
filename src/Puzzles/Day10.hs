...
getOnesAndThrees i = foldr
    (\x ( ones, threes ) -> if x == 1 then ( ones + 1, threes )
     else if x == 3 then ( ones, threes + 1 ) else ( ones, threes )) ( 0, 0 )
    $ getOutputs i
  ...
  where
    ...
    go i
        | i == (V.length os - 1) = do modify @IMap (M.insert i 1)
                                      pure 1
        | otherwise = do
            p <- gets @IMap (M.lookup i)
            maybe (do t <- sum <$> sequence [ plus1, plus2, plus3 ]
                      t <$ modify @IMap (M.insert i t)) pure p
      where
        plus1
            | os V.! i <= 3 = go (i + 1)
            | otherwise = pure 0
